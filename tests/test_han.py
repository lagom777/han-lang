...
def test_블록_스코프_대입():
    # 블록 안에서 처음 대입한 변수가 블록 밖에서도 살아있어야 함(스코프 누수 버그 수정)
    assert run('만약 참 { 답 = "만약" }\n출력(답)') == "만약\n"
    assert run('만약 거짓 { } 아니면 { 답 = "아니면" }\n출력(답)') == "아니면\n"
    assert run('동안 참 { 답 = "동안"\n멈춤 }\n출력(답)') == "동안\n"
    assert run('시도 { 답 = "시도" } 잡기(오류) { }\n출력(답)') == "시도\n"
    # 회귀: 바깥 변수를 블록 안에서 갱신하면 제자리(바깥 스코프)에서 갱신돼야 함
    assert run('답 = 1\n만약 참 { 답 = 2 }\n출력(답)') == "2\n"
...
def test_한쪽다듬기():
    # 왼다듬기=lstrip(앞만), 오른다듬기=rstrip(뒤만), 둘 합치면 다듬기(양쪽)
    assert run('출력("[" + 왼다듬기("  가 ") + "]")') == "[가 ]\n"
    assert run('출력("[" + 오른다듬기(" 가  ") + "]")') == "[ 가]\n"
    assert run('출력(오른다듬기(왼다듬기("  가나 ")) == 다듬기("  가나 "))') == "참\n"
...
def test_공백인가():
    # 모두 공백(빈칸·탭·개행)이면 참, 내용이 있거나 빈 문자열·비문자열은 거짓(빈칸 입력 검증)
    assert run('출력(공백인가("   "))') == "참\n"
    assert run('출력(공백인가("\t\n"))') == "참\n"
    assert run('출력(공백인가("가"))') == "거짓\n"
    assert run('출력(공백인가("a "))') == "거짓\n"          # 내용 섞이면 거짓
    assert run('출력(공백인가(""))') == "거짓\n"            # 빈 문자열 거짓
    assert run('출력(공백인가(123))') == "거짓\n"           # 비문자열 거짓
